...
export const AppBar = ({ user, onSignin, onSignout, avatar } : Appbar) => {
  ...
  return (
    <ul className="flex  justify-between w-screen px-4 h-[70px] fixed z-50">
      <li className="text-3xl font-black text-[#fff] flex flex-col justify-center text-[#cac4ce]">
        <Link href={"/"}>CoinWallet</Link>
      </li>
      <li className="bg-[#b7acbf] w-[18%] h-[70%] zindex-100 hover:w-[30%] duration-700 font-bold text-lg rounded-[90px] flex items-center justify-end m-2 px-10 shadow-xl">
        {username?.split(' ').at(0)}
        {/* {!username && <RotatingWords />} */}
        {!username && <div className="italic">Welcome</div>}
        {username && <div className="bg-[#000] w-[40px] h-[40px] rounded-full mr-6 ml-4 hover:cursor-pointer" onClick={handleProfile}>
        ...
        {username && <div className="hover:scale-110">
          <Bell />
        </div>}
      </li>
